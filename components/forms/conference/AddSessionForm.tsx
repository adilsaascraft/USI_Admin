...
export default function AddSessionForm({
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const [d, h, t, s] = await Promise.all([
        fetch(
          `${process.env.NEXT_PUBLIC__URL}/admin/conferences/${conferenceId}/dates`,
          { headers }
        ),
        fetch(
          `${process.env.NEXT_PUBLIC__URL}/conferences/${conferenceId}/halls/active`,
          { headers }
        ),
        fetch(
          `${process.env.NEXT_PUBLIC__URL}/conferences/${conferenceId}/tracks/active`,
          { headers }
        ),
  ...
  async function onSubmit(data: SessionValues) {
    try {
      ...
      const url = isEdit
        ? `${process.env.NEXT_PUBLIC__URL}//admin/sessions/${defaultValues!._id}`
        : `${process.env.NEXT_PUBLIC__URL}//admin/conferences/${conferenceId}/sessions`
